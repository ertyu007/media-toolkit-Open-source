...
### เว็บไซต์เปลี่ยนระบบจนตัวดาวน์โหลดตามไม่ทัน (เช่น TikTok `Unexpected response from webpage request`)

หากข้อความแจ้งว่า "เว็บไซต์เปลี่ยนระบบจนตัวดาวน์โหลดตามไม่ทัน" หมายความว่า yt-dlp อ่านหน้าเว็บนั้นไม่ได้แล้ว (เช่น TikTok เปลี่ยนหน้า challenge) ไม่ใช่ลิงก์ผิดเสมอไป:

- กด **อัปเดต yt-dlp (Ctrl+U)** ในแอปแล้วลองใหม่เป็นอันดับแรก
- หากอัปเดตแล้วยังไม่ได้ ให้รอ yt-dlp ออกเวอร์ชันใหม่แล้วลองอีกครั้ง Clipora จะไม่ retry ซ้ำเพราะต้องแก้ extractor ต้นทางก่อน
- ตรวจว่าลิงก์ยังเปิดแบบสาธารณะได้อยู่ และอย่าใช้ login/cookies/private media ซึ่งอยู่นอกขอบเขตของ Clipora

### โดนบล็อก (HTTP 403 / "not a bot" / 429)

Clipora จะลองดาวน์โหลดซ้ำอัตโนมัติด้วยวิธีที่เพิ่มขึ้น: เพิ่ม header เฉพาะเว็บไซต์ (เช่น Referer ของ TikTok), สลับ player client ของ YouTube (android/web_embedded/tv) แล้วใช้การจำลองเบราว์เซอร์ (`--impersonate`) ก่อนจะรายงานว่าล้มเหลว:

- หากข้อความแจ้งว่า "เครือข่าย/ISP บล็อกการเข้าถึงเว็บไซต์นี้ (resolve โดเมนไม่ได้)" หมายความว่าถูกบล็อกที่ระดับเครือข่าย/ISP (เช่น DNS ถูกสลับหรือดัก) ไม่ใช่ตัวโปรแกรม ให้ลองเปลี่ยน DNS เป็น `1.1.1.1` หรือ `8.8.8.8` หรือใช้ VPN/proxy แล้วลองใหม่ Clipora จะไม่พยายาม retry ซ้ำเพราะวิธีจำลองเบราว์เซอร์ช่วยไม่ได้
- หากข้อความแจ้งว่า "เว็บไซต์บล็อกการดาวน์โหลดอัตโนมัติ" หลังลองทุกวิธีแล้ว หมายความว่าเว็บนั้นบล็อกด้วยวิธีที่แรงกว่า (เช่น ตรวจ IP หรือต้องใช้คุกกี้/เข้าสู่ระบบ) ซึ่งอยู่นอกขอบเขตของ Clipora
- ตรวจว่าเครือข่ายเข้าถึงเว็บไซต์ได้จริง เช่น `nslookup www.youtube.com` — ถ้า DNS resolve ไม่ได้ หมายความว่าถูกบล็อกที่ระดับเครือข่าย/ISP ไม่เกี่ยวกับตัวโปรแกรม ให้ลองใช้เครือข่ายหรือ VPN อื่น
- อัปเดต yt-dlp ก่อนลองใหม่ เพราะ extractor ถูกปรับบ่อยตามที่เว็บเปลี่ยน
...
Clipora จะถามให้เลือก **เขียนทับ**, **เก็บทั้งสองไฟล์** หรือ **ยกเลิก** ก่อนเริ่มงาน — เลือกเก็บทั้งสองไฟล์เพื่อให้ได้ชื่อ `(1)`, `(2)` โดยไม่ลบไฟล์เดิม

### เปิดไฟล์ผลลัพธ์ไม่ได้ "คุณไม่มีสิทธิ์เข้าถึงไฟล์นี้" (0x80070005)

กรณีไฟล์ที่ดาวน์โหลดจากลิงก์เปิดไม่ได้ด้วย error `0x80070005` (E_ACCESSDENIED) — เกิดจากไฟล์ถูกสร้างด้วย ACL ที่ผู้ใช้ปัจจุบันไม่มีสิทธิ์ (owner SID ต่าง / ไม่อนุญาตอ่าน):

- Clipora ตั้งแต่รุ่นนี้จะรีเซ็ตสิทธิ์ของไฟล์ผลลัพธ์ให้ผู้ใช้ปัจจุบันทุกครั้ง (ผ่าน `icacls`) จึงไม่เกิดซ้ำกับไฟล์ใหม่
- ไฟล์ที่ดาวน์โหลดมาก่อนหน้านี้แล้วเปิดไม่ได้ ให้แก้ ACL ด้วยคำสั่ง (รัน PowerShell แบบ Administrator):

```powershell
takeown /f "C:\Users\<ชื่อผู้ใช้>\Downloads\ชื่อไฟล์.mp4"
icacls "C:\Users\<ชื่อผู้ใช้>\Downloads\ชื่อไฟล์.mp4" /grant "$env:USERDOMAIN\$env:USERNAME:(F)"
```

- หรือลบไฟล์นั้นแล้วดาวน์โหลดใหม่ด้วย Clipora เวอร์ชันนี้
