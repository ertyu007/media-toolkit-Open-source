...
4. เปิด Clipora ครั้งแรก แล้วกด **ติดตั้งเครื่องมือที่จำเป็น**
5. รอ Setup Assistant ดาวน์โหลด FFmpeg, yt-dlp และ Deno จาก release ทางการ ตรวจ SHA-256 และติดตั้งลง `%LOCALAPPDATA%\Clipora\tools`
6. เมื่อขึ้น **Clipora พร้อมใช้งานแล้ว** ให้วางลิงก์และเริ่มงานได้ทันที
...
Setup Assistant ไม่ดาวน์โหลดไฟล์แบบเงียบ ผู้ใช้ต้องกดยืนยันก่อน ทุก URL ใช้ HTTPS มีขีดจำกัดขนาด และตรวจ checksum ที่ pin ไว้ก่อนติดตั้ง
...
ต้องใช้ Python 3.10+ และ Inno Setup 6 ผลลัพธ์อยู่ที่ `dist\installer` การ push tag เช่น `v0.4.0` จะให้ GitHub Actions ทดสอบ สร้าง Setup และแนบ checksum ไปยัง GitHub Release อัตโนมัติ
